...
export class MainPage {
  ...
  async checkElementsHrefAttribute() {
    for (const { locator, name, text, attribute } of this.elements) {
      if (attribute) {
  ...
  async clickSwitchLightModeIkon() {
  ...
  async buttonGetStartedIsVisible() {
    await expect(this.page.getByRole('link', { name: 'Get started' })).toBeVisible();
  }

  async buttonGetStartedContainsText() {
    await expect(this.page.getByRole('link', { name: 'Get started' })).toContainText('Get started');
  }

  async buttonGetStartedHasAttributeHref() {
    await expect(this.page.getByRole('link', { name: 'Get started' })).toHaveAttribute(
      'href',
      '/docs/intro',
    );
  }
  ...
  async headingIsVisible() {
    await expect(
      this.page.getByRole('heading', { name: 'Playwright enables reliable' }),
    ).toBeVisible();
  }

  async headingHasExpectedText() {
    await expect(
      this.page.getByRole('heading', { name: 'Playwright enables reliable' }),
